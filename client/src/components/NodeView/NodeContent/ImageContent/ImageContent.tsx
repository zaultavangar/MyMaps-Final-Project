...
export const ImageContent = () => {
  ...
  /* Function to resize images using updateNode */
  const resizeImage = async (
    originalWidth: NodeFields,
    newWidth: number,
    originalHeight: NodeFields,
    newHeight: number
  ) => {
    // Make INodeProperty for width and height, use to update the node
    const widthNodeProperty: INodeProperty = makeINodeProperty(originalWidth, newWidth)
    const heightNodeProperty: INodeProperty = makeINodeProperty(originalHeight, newHeight)
    const updateSizeResp = await FrontendNodeGateway.updateNode(currentNode.nodeId, [
      widthNodeProperty,
      heightNodeProperty,
    ])
    if (!updateSizeResp.success) {
      setError('Error in updating node')
    }
  }

  /* Resize the image when the updated width and height change */
  useEffect(() => {
    resizeImage('updatedWidth', updatedWidth, 'updatedHeight', updatedHeight)
  }, [updatedWidth, updatedHeight])

  return (
    <div className="imageWrapper">
      <div className="imageResizeMenu">
        <Button
          text={'Reset crop'}
          icon={<ri.RiArrowGoBackFill />}
          onClick={() => {
            // Revert to original size if button is pressed
            setUpdatedWidth(currentNode.originalWidth ?? 0)
            setUpdatedHeight(currentNode.originalHeight ?? 0)
          }}
          style={{ height: '40px' }}
        />
        <NumberInputComponent
          value={updatedWidth}
          setValue={setUpdatedWidth}
          max={currentNode.originalWidth ?? 0}
        />
        <NumberInputComponent
          value={updatedHeight}
          setValue={setUpdatedHeight}
          max={currentNode.originalHeight ?? 0}
        />
      </div>
      <div
        ref={imageContainer}
...
const NumberInputComponent = (props: INumberInputComponentProps): JSX.Element => {
  ...
  const pxFormat = (value: number) => Math.floor(value) + 'px'

  // Following ChakraUI documentation to create the Input text options
  return (
    <NumberInput
      maxW="150px"
