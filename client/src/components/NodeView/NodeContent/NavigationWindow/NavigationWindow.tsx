...
export const NavigationWindow = (props: INavigationWindowProps) => {
  ...
  const handleBackNavigationClick = () => {
    if (currentNavigationPin) {
      ...
      let index = trailToNavigate.pinList.indexOf(currentNavigationPin)
      if (index <= 0) return
  ...
  var isFirst: boolean = trailToNavigate.pinList[0] === selectedPin
  var isLast: boolean = trailToNavigate.pinList[trailToNavigate.pinList.length-1] === selectedPin
        ...
        <div className="nav-wind-pins-header">Pins</div>
          {currentNavigationPin !== null ? 
              <div style={{display: 'flex', alignItems: 'center', gap: '2px'}}>
                  {!isFirst &&
                    <button className='navigate-back-button' onClick={handleBackNavigationClick}>
                  ...
                  {!isLast &&
                  <button className='navigate-next-button' onClick={handleNextNavigationClick}>
                    <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
                      <NavigateNextIcon />
                    </div>
                  </button >
                  }
                  <button className='navigate-restart-button' onClick={handleRestartButtonClick}>
                    <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
                      <RestartAltIcon/>
                    </div>
                  </button>
              ...
              :
              <button className="nav-wind-navigate-button" onClick={handleStartNavigationClick}>
